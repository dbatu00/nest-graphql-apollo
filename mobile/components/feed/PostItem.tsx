...
export function PostItem({ post, currentUserId, onDelete, onToggleFollow, onToggleLike }: Props) {
  const isOwner = currentUserId === post.user.id;

  return (
    <View style={feedStyles.postCard}>
      <UserRow
        user={{ id: post.user.id, username: post.user.username, displayName: post.user.username, followedByMe: post.user.followedByMe }}
      ...
      <View style={feedStyles.footer}>
        <Text style={feedStyles.timestamp}>{new Date(post.createdAt).toLocaleString()}</Text>
        {onToggleLike && (
          <LikeButton
            postId={post.id}
